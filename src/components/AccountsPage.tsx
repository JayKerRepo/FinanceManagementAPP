...
export default function AccountsPage({ onNavigate, currentPage = 'accounts' }: Props = {}) {
  ...
  const getBreakdown = (period: typeof reportPeriod, biz: 'all' | string) => {
    ...
    const filteredTx = tx.filter((t: { date?: string; transaction_type?: string }) => {
  ...
  const totalBalance = accounts
    .filter((acc: { business_id?: string }) => selectedFilter === 'all' || acc.business_id === selectedFilter)
    .reduce((sum, acc: { balance?: number }) => sum + (acc.balance || 0), 0);
  ...
  const getThisMonthBreakdown = () => {
    ...
    const monthTx = tx.filter((t: { date?: string; transaction_type?: string }) => {
      if (!t.date) return false;
      const txDate = new Date(t.date);
    ...
    return Object.entries(categoryTotals)
      ...
      .map((item, _, arr) => {
        const total = arr.reduce((sum, i: { amount: number }) => sum + i.amount, 0);
  ...
  const getExpenseTrends = () => {
    const tx = selectedFilter === 'all' ? allTransactions : transactions.filter((t: { business_id?: string }) => t.business_id === selectedFilter);
    const expenses = tx.filter((t: { transaction_type?: string }) => t.transaction_type === 'expense');
    ...
    const currentWeek = expenses.filter((t: { date?: string; amount?: number }) => {
      if (!t.date) return false;
      const txDate = new Date(t.date);
      return txDate >= weekStart && txDate <= now;
    });
    const currentWeekTotal = currentWeek.reduce((sum, t: { amount?: number }) => sum + (t.amount || 0), 0);
    
    // Previous week
    const prevWeekStart = new Date(weekStart);
    prevWeekStart.setDate(prevWeekStart.getDate() - 7);
    const previousWeek = expenses.filter((t: { date?: string; amount?: number }) => {
      if (!t.date) return false;
      const txDate = new Date(t.date);
      return txDate >= prevWeekStart && txDate < weekStart;
    });
    const previousWeekTotal = previousWeek.reduce((sum, t: { amount?: number }) => sum + (t.amount || 0), 0);
    
    // Current month
    const monthStart = new Date(now.getFullYear(), now.getMonth(), 1);
    const currentMonth = expenses.filter((t: { date?: string; amount?: number }) => {
      if (!t.date) return false;
      const txDate = new Date(t.date);
      return txDate >= monthStart && txDate <= now;
    });
    const currentMonthTotal = currentMonth.reduce((sum, t: { amount?: number }) => sum + (t.amount || 0), 0);
    
    // Previous month
    const prevMonthStart = new Date(now.getFullYear(), now.getMonth() - 1, 1);
    const prevMonthEnd = new Date(now.getFullYear(), now.getMonth(), 0);
    const previousMonth = expenses.filter((t: { date?: string; amount?: number }) => {
      if (!t.date) return false;
      const txDate = new Date(t.date);
      return txDate >= prevMonthStart && txDate <= prevMonthEnd;
    });
    const previousMonthTotal = previousMonth.reduce((sum, t: { amount?: number }) => sum + (t.amount || 0), 0);
  ...
  const getTopCategoriesWithTrends = () => {
    ...
    const prevTx = tx.filter((t: { date?: string; transaction_type?: string }) => {
      if (!t.date || t.transaction_type !== 'expense') return false;
      const txDate = new Date(t.date);
    ...
    const total = Object.values(categoryTotals).reduce((sum, cat: { amount: number }) => sum + cat.amount, 0);
